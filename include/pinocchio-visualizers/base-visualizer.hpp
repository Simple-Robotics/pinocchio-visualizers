...
namespace pinocchio_visualizers
{
  ...
  /// @brief A base class for defining visualizers for Pinocchio in C++. This provides basic building blocks
  /// (a base constructor, data members, getters for the models).
  /// @details The base API assumes that the visualizer is not the owner of the underlying
  /// Pinocchio multibody, visual or collision models.
  /// Their lifetimes should be managed by the application context itself.
  /// @remark C++ port of the BaseVisualizer abstract class in Pinocchio's Python bindings.
  ...
  public:
    ...
    /// @brief Display configuration @p q (if an actual value is given) or update the
    /// Pinocchio frames.
    virtual void display(const std::optional<ConstVectorRef> &q = std::nullopt);

    /// @brief Play an entire trajectory, waiting for time @p dt between each keyframe.
    ...
    virtual void
    setCameraTarget(const Eigen::Ref<const Vector3s> & /*target*/) {}

    /// @brief Set the active camera position.
    virtual void
    setCameraPosition(const Eigen::Ref<const Vector3s> & /*position*/) {}

    /// @brief Set the active camera 6D pose.
    virtual void
    setCameraPose(const Eigen::Ref<const Matrix4s> & /*pose*/) {}
    ...
    /// @brief Set camera zoom level; what this means depends on the implementation
    /// (FOV zoom or moving forwards).
